namespace KBEngine
{
	...
	*/
    public class Event
    {
		...
		//20170904 hyc
		static Dictionary<string,LinkedList<EventObj>> cache_fireout = new Dictionary<string, LinkedList<EventObj>>();
		...
		*/
		public static void fireOut(string eventname, params object[] args)
		{
			if (_isPauseOut) {
				//20170904 hyc
				LinkedList<EventObj> lst = null;
				if (!cache_fireout.TryGetValue(eventname,out lst)) {
					lst = new LinkedList<EventObj>();
					EventObj obj = new EventObj ();
					obj.args = args;
					lst.AddLast (obj);
					cache_fireout.Add (eventname, lst);
				} else {
					EventObj obj = new EventObj ();
					obj.args = args;
					lst.AddLast (obj);
		...
		*/
		...
		{
			//20170904 hyc
			if (cache_fireout.Count > 0 && !_isPauseOut) {
				foreach (string key in cache_fireout.Keys) {
					var iter = cache_fireout [key].GetEnumerator ();
					while (iter.MoveNext()) {
						fireOut (key, iter.Current.args);
					}
					cache_fireout [key].Clear ();
				}
				cache_fireout.Clear ();
			}
...
//				Debug.LogError("processOutEvents:" + eobj.info.funcname + "(" + eobj.info + ")");
				foreach(object v in eobj.args)
				{
//					Debug.LogError("processOutEvents:args=" + v);
				}
				try
				{
		...
		//20170904 hyc
		static LinkedList<EventObj> nocache_Fireout = new LinkedList<EventObj>();
